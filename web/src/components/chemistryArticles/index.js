...
function ChemistryArticles(props) {
  const FETCH_BIA_CHEMISTRY = gql`
    query bia($start: Int!) {
      chemistry(start: $start) {
        data
        titulo
        link
        autores
  ...
  `;
  const { loading, data } = useQuery(FETCH_BIA_CHEMISTRY, {
