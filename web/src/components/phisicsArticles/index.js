...
function PhisicsArticles(props) {
  const FETCH_BIA_PHISICS = gql`
    query bia($start: Int!) {
      phisics(start: $start) {
        data
        titulo
  ...
  `;
  const { loading, data } = useQuery(FETCH_BIA_PHISICS, {
...
export default PhisicsArticles;